...
![Posterior distributions of average penguin weights](images/post-dist-peng1.png){#fig-post-peng1 fig-align="center" width=3.9in
height=3in}
...
The columns we met before capture different parts of the histograms we saw in
@fig-post-peng1. `Estimate` is the mean of the histogram (shown as black
dots), `Est.Error` is the standard error of these histograms, and l-95% CI and
u-95% CI are the 5% and 95% quantiles of the histograms (shown as black bars).
Other terms need elaboration, like chain, iter, warmup, Rhat, and Bulk_ESS
...
of the center (or bulk) of our posterior distribution, e.g., the number we see
in the `Estimate` column. **Tail ESS** measures the reliability of the tails of
the posterior distribution, e.g., the numbers in columns `l-95% CI` and
`u-95% CI`. So, having both a high Bulk ESS and high Tail ESS suggest that our
histograms as a whole are reliable.
...
A more elegant plot with simpler code combines `ggplot` with the
`stat_lineribbon()` function from the `tidybayes` library. `stat_lineribbon()`
takes all the regression draws we saved in `posterior_lines` and, with some
trickery, transforms them into a smooth plot. The thick lines represent the
means and the shaded regions represent the credibility intervals of the average
bill length at each weight.
...
ggplot(
  ...
  stat_lineribbon(.width = 0.95, point_interval = "mean_qi", alpha = 0.3) +
...
![](images/bill-len-plt.png){#fig-bill-reg2 fig-align="center" width=600}
