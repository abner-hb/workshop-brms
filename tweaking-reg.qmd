...
These numerical summaries have enriched their meaning: they capture different parts of the histograms in @fig-post-peng1. `Estimate` shows their mean
(painted as black dots), `Est.Error` shows their standard errors, and
...
iterations, including warmup. Combining the post-warmup draws from all four
chains gives us 4000 useful draws in total.
...
the penguins, which in turn changes by sex and species. @fig-bill-len-assocs
...
ggplot(
  ...
  geom_point() +
  labs(
    title = "Males and heavier penguins have longer bills",
    x = "Body mass (kg)",
    y = "Bill length (mm)"
  ) +
  theme_bw() +
